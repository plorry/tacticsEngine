...
var Cursor = exports.Cursor = TileSprite.extend({
    initialize: function(options) {
        options.image = conf.Images.cursor;
...
var Unit = exports.Unit = TileSprite.extend({
    ...
    checkTerrainDifficulty: function(tile) {
        ...
        // This method is left blank intentionally, to be overridden by specific units
        return 0;
    },
...
var getTilesInRange = exports.getTilesInRange = function(unit, map, coords, maxDistance, minDistance, cantPass, cantRest) {
    /*
    ...
    for(var depth = 1; depth <= maxDistance; depth++) {
        ...
        tiles_ordered[depth - 1].forEach(function(tile){
            ...
            neighbours.forEach(function(neighbour) {
                ...
                if (neighbour.properties.obstacle === true) {
                    available = false;
                }
                if (available) {
                    if (!unit.checkTerrainDifficulty(neighbour)) {
                        tiles_ordered[depth].push(neighbour);
                    } else {
                        var difficulty = unit.checkTerrainDifficulty(neighbour);
                        if (!tiles_ordered[depth + difficulty]) {
                            tiles_ordered[depth + difficulty] = [];
                        }
                        tiles_ordered[depth + difficulty].push(neighbour);
                    }
                }
